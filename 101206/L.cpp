...
int segundo_domingo(int y, int m, int day_max, int target){
    int domingos_vistos = 0;
    forr(i, 1, day_max+1){
        if(weekday(y, m, i) == 6){
            domingos_vistos++;
            if(domingos_vistos == target)
                return i;
        }
    }
...
int main(){  
    ...
    forr(este_test, 1, tc+1){
        ...
        if(month == 3){ // Marzo
            int day_segundo_domingo = segundo_domingo(year, month, 31, 2);
            dbg(day_segundo_domingo);
            ...
            int day_segundo_domingo = segundo_domingo(year, month, 30, 1);
            dbg(day_segundo_domingo);
            if(day == day_segundo_domingo){
                if(hour < 1) ans = 2;
