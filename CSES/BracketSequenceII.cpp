...
ll catalan(ll n){ return combinatorio(2*n, n) * inv_mod(n+1, MOD) % MOD; }
...
int main(){  
	...
	forn(i, s.size()){
		...
		if(close > open || 2*open > n || n&1){
			cout << "0\n";
			return 0;
		}
	}

	ll h = n/2 - close, w = n/2 - open;
	...
	ll ans = ((combinatorio(h + w, w) - combinatorio(h + w, h+1))%MOD + MOD)%MOD;
