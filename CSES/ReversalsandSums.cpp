...
ll get_sum(pitem &root, int l, int r) {
  ...
  ll ans = aux1->sum; // aux1->mini for minimum
  merge(root,root,aux1); merge(root,root,aux2);
  return ans;
}
...
int main(){  
    ...
    pitem root = NULL;
    forn(i, n) merge(root, root, new item(a[i]));
    ...
    forn(i, q){
    	...
    	if(t == 1){
    		...
    		split(root, first_part, middle, l);
    		split(middle, middle, second_part, r-l+1);
    		middle->rev ^= 1;
    		merge(root, first_part, middle);
    		merge(root, root, second_part);
    	} else {
    		assert(t == 2);
